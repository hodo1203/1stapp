...
if menu == "1. 삼각함수 그래프 & 역함수 탐구":
    ...
    st.markdown("상단에서 함수의 종류, 진폭, 주기, 평행이동 등을 조절하여 **그래프의 위치가 변환**되는 것을 확인해보세요.")
    ...
    # 함수 값 계산 (수직이동은 y값 자체에 더해져 그래프 전체 위치를 위/아래로 이동시킴)
    if "사인" in func_type:
        y = amplitude * np.sin(period_mult * x - phase_shift) + vertical_shift
        col_name = f"y = {amplitude} * sin({period_mult}*x) + {vertical_shift}"
    elif "코사인" in func_type:
        y = amplitude * np.cos(period_mult * x - phase_shift) + vertical_shift
        col_name = f"y = {amplitude} * cos({period_mult}*x) + {vertical_shift}"
    else:
        y = amplitude * np.tan(period_mult * x - phase_shift) + vertical_shift
        y[np.abs(np.gradient(y)) > 30] = np.nan  # 점근선 처리
        col_name = f"y = {amplitude} * tan({period_mult}*x) + {vertical_shift}"

    # 스트림릿 내장 선 그래프 시각화 (x축 기준)
    chart_data = pd.DataFrame({col_name: y}, index=x)
...
# ---------------------------------------------------------
    ...
    st.markdown("슬라이더를 통해 각도(라디안)를 조절하고, 단위 원 위의 좌표와 삼각함수 값을 확인해보세요.")
    ...
    col2.metric("코사인 값 (x 좌표)", f"{cos_val:.4f}")
    col3.metric("사인 값 (y 좌표)", f"{sin_val:.4f}")
