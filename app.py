...
if menu == "1. 삼각함수 그래프 & 역함수 탐구":
    ...
    st.markdown("상단에서 함수의 종류, 진폭, 주기, 평행이동 등을 조절하고, 하단에서 특정 $y$값에 해당하는 $x$값을 그래프에서 확인해보세요.")
    ...
    if "사인" in func_type:
        ...
        title_str = f"y = {amplitude} \\sin({period_mult}x) + {vertical_shift}"
    elif "코사인" in func_type:
        y = amplitude * np.cos(period_mult * x - phase_shift) + vertical_shift
        title_str = f"y = {amplitude} \\cos({period_mult}x) + {vertical_shift}"
    else:
        y = amplitude * np.tan(period_mult * x - phase_shift) + vertical_shift
        y[np.abs(np.gradient(y)) > 50] = np.nan
        title_str = f"y = {amplitude} \\tan({period_mult}x) + {vertical_shift}"

    fig, ax = plt.subplots(figsize=(10, 5))
    ax.plot(x, y, label=title_str, color='purple', linewidth=2)
    ax.axhline(0, color='black', linewidth=1, linestyle='--')
    ax.axvline(0, color='black', linewidth=1, linestyle='--')
    ax.set_title(f"Graph: {title_str}", fontsize=14)
    ...
    if found_x:
        for fx in found_x:
            ax.plot(fx, target_y, 'ro', markersize=8)
            ax.annotate(f"x = {fx:.3f} rad", (fx, target_y), textcoords="offset points", xytext=(0,10), ha='center', color='red', fontweight='bold')
    ...
    ax.axhline(target_y, color='blue', linestyle='-.', alpha=0.5, label=f"y = {target_y}")
...
# ---------------------------------------------------------
    ...
    with col_r:
        ...
        if show_sin:
            ax_f.plot(x_vals, np.sin(x_vals), 'g-', label='y = sin(x)', alpha=0.6)
            ax_f.plot(theta, np.sin(theta), 'go', markersize=8)
        if show_cos:
            ax_f.plot(x_vals, np.cos(x_vals), 'r-', label='y = cos(x)', alpha=0.6)
            ax_f.plot(theta, np.cos(theta), 'ro', markersize=8)
        if show_tan:
            tan_vals = np.tan(x_vals)
            tan_vals[np.abs(np.gradient(tan_vals)) > 20] = np.nan
            ax_f.plot(x_vals, tan_vals, 'b-', label='y = tan(x)', alpha=0.6)
            if np.cos(theta) != 0:
                ax_f.plot(theta, np.tan(theta), 'bo', markersize=8)

        ax_f.axvline(theta, color='orange', linestyle='--', label=f'현재 각도 (x = {theta:.2f})')
...
# ---------------------------------------------------------
    ...
    data = {
        ...
        "탄젠트 (tan)": ["0", "$\\frac{\\sqrt{3}}{3}$", "1", "$\\sqrt{3}$", "존재하지 않음 (정의되지 않음)"]
    }
...
# ---------------------------------------------------------
    ...
    st.markdown("각변환 공식($\\pi \\pm \\theta, \\frac{\\pi}{2} \\pm \\theta$ 등)을 학습하기 위한 퀴즈입니다. 틀리면 한 번 더 기회가 제공되고, 재도전 시에도 틀리면 오답과 해설이 제공됩니다.")
